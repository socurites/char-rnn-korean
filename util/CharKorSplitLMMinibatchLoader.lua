...
-- the modification includes support for utf8
...
function CharSplitLMMinibatchLoader.text_to_tensor(in_textfile, out_vocabfile, out_tensorfile)
    ...
    repeat
        for char in utf8.gmatch(rawdata, '.') do
            if not unordered[char] then unordered[char] = true end
        end
        tot_len = tot_len + utf8.len(rawdata)
        rawdata = f:read(cache_len)
    until not rawdata
    f:close()
    ...
    repeat
        for i=1, utf8.len(rawdata) do
            data[currlen+i] = vocab_mapping[utf8.sub(rawdata, i, i)] -- lua has no string indexing using []
        end
        currlen = currlen + utf8.len(rawdata)
        rawdata = f:read(cache_len)
    until not rawdata
    f:close()
